...
-- ── Permissions (97 baris) ──
INSERT INTO "auth"."permissions" ("id","resource","action","kode","nama","modul") VALUES
  (gen_random_uuid(), 'dashboard.view', 'read', 'dashboard.view:read', 'Dashboard Operasional — Lihat', 'dashboard'),
...
INSERT INTO "auth"."role_permissions" ("role_id","permission_id")
SELECT r."id", p."id" FROM "auth"."roles" r
  JOIN "auth"."permissions" p ON p."kode" IN ('dashboard.view:read', 'registration.pasien:read', 'registration.pasien:create', 'registration.pasien:update', 'registration.kunjungan:read', 'registration.kunjungan:create', 'registration.kunjungan:update', 'clinical.rj:read')
