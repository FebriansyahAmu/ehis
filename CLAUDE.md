...
- [x] **Log Dosis Radiasi (DRL Monitoring)** ✅ — `manajemen/DosisPane.tsx`. Two-panel: kiri dose log list (filter per modalitas). Kanan: detail entry (exceeded alert rose, patient info, DRLGauge animated per parameter: CTDIvol/DLP/entrance dose, ALARA reminder card). `DRLSummary` panel (total/exceeded/%, rata-rata CTDIvol, DRL referensi tabel). `DRL_CT` + `DRL_ENTRANCE` PMK 1014/2008 values. 12 mock entries `DOSIS_LOG_MOCK`. Perka BAPETEN No. 2/2018 · IAEA Safety Reports 39
