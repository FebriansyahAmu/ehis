...
| `/ehis-eklaim`             | E-Klaim          | 🚧 **EK0 ✅ · EK1 ✅ · EK2 ✅ · EK3 ✅ · EK4 ✅ · EK5 ✅ · EK6 ✅ · EK7 🚧 75% · EK8 🚧 50%** — EK7 Reconciliation EK7.1+EK7.2+EK7.3 ✅ (`ReconciliationPage` 2-panel · `ImportTransferModal` CSV drag-drop · `MatchingPanel` 3-strategy engine · `SelisihWriteOffModal` 2-step Write-off/Refund/Pending) · **EK8 Dashboard** EK8.1+EK8.2+EK8.3 ✅: `DashboardPage` 2-panel (left nav 3 tab + right AnimatePresence) · KPI strip 4 cards · `ApprovalRatePanel` SVG line chart + rejected reasons · `AgingKlaimPanel` stacked bars + stuck claims table · `MarginAnalysisPanel` diverging bar chart + caveat · route `/report` · `dashboardShared.ts` pure builders · skeleton 500ms · TSC clean. **iDRG-first**. Next: EK7.4 Reconciliation Report. Roadmap [TODO-EKLAIM.md](TODO-EKLAIM.md) |
...
- [🚧] **`ehis-eklaim`** Klaim — modul baru hasil scope-split dari billing (workflow batch + persona Tim Klaim/Coder berbeda). **Roadmap [TODO-EKLAIM.md](TODO-EKLAIM.md)** (39 task, fase EK0–EK9 · ~17/39 = 44%). **EK0 ✅ · EK1 ✅ · EK2 ✅ · EK3 ✅ · EK4 ✅ · EK5 ✅ · EK6 ✅ 100% · EK7 🚧 75% · EK8 🚧 50%**. EK8.1+EK8.2+EK8.3 Dashboard done: `DashboardPage` 2-panel · `ApprovalRatePanel` SVG line chart · `AgingKlaimPanel` stacked bars · `MarginAnalysisPanel` diverging bars · `dashboardShared.ts` pure builders · route `/report` · TSC clean. **Next: EK7.4 Reconciliation Report**.
