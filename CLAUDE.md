...
- [ ] **Kewenangan Klinis** — Dokter ↔ Tindakan. Matrix 2D: row=Dokter, col=Tindakan/Kelompok Tindakan, cell=checkbox (boleh/tidak). Filter per spesialis. Important: SNARS PMK 755 — dokter hanya boleh tindakan yang ter-credentialed. Depends on Katalog Tindakan (T3 Tarif).
- [ ] **Layanan Unit** — Tindakan ↔ Unit. Tindakan apa boleh dilakukan di Unit mana (mis. Operasi Besar hanya di OK, bukan di Poli). Matrix sederhana. Depends on Katalog Tindakan.
- [ ] **Tarif Matrix** — Tindakan × Penjamin × Kelas → Tarif. 3D matrix paling complex. Default view: pivot table dengan tabs per Penjamin (Umum / BPJS / Asuransi Swasta), row=Tindakan, col=Kelas, cell=harga. Bulk update (mis. naikkan 5% semua tarif IRJA). Import/export Excel. Depends on Katalog Tindakan + Penjamin (T3).
- [ ] **Formularium Penjamin** — Obat × Kelas Penjamin → boleh/tidak. BPJS formularium nasional + RS formularium tambahan. Matrix row=Obat, col=Kelas, cell=boleh/tidak + alasan substitusi. Depends on Katalog Obat (T2) + Penjamin (T3).
- [ ] **Distribusi Obat** — Obat ↔ Depo Farmasi. Obat mana di-stock di depo mana (Depo IGD vs Apotek RI vs Apotek RJ vs Gudang Pusat). Re-stock alert per-depo. Depends on Katalog Obat (T2) + Depo (Gudang Farmasi tech debt).
- [ ] **RBAC (Role × Permission)** — Role (9 di `penggunaShared.ts`) × Permission (read/write/sync per modul + per tab). Matrix dengan permission tree (CRUD per resource + module-specific). Saat ini hard-coded di tiap form, akan jadi konfigurabel.

> Cross-cutting: tambahkan label "Source: Mapping Hub" di shortcut field entitas (DokterDetail.poliAssignment, dst) supaya user paham source of truth. Audit trail per relasi (siapa edit kapan) — kandidat shared component `mapping/MappingAuditPane.tsx`.
