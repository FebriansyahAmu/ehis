...
8. - [x] **Discharge Planning** (`tabs/DischargePlanTab.tsx`) — ✅ **Rebuild lengkap** — 4-step stepper dengan phase indicator (Fase 1 Hari 1-2 MRS / Fase 2 Sepanjang Rawat / Fase 3 H-1 Pulang / Fase 4 Hari Pulang), DischargeHeader (hari perawatan, KRS target, readiness % bar), Framer Motion direction-aware transitions, per-day education logs (EdukasiLog per topik), StepRencanaPulang merged (obat pulang + jadwal kontrol + jadwal pemeriksaan + FKTP), DPJP sign-off, print preview PMK 24/2022, phase banner sky/emerald/amber/orange per step. Mock data RM-2025-003 with 7-day history. Sub: `discharge/{dischargeShared,StepAsesmen,StepEdukasi,StepRencanaPulang,StepResumeMedis}.tsx`
