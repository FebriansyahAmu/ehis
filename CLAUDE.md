...
| [TODO-BPJS.md](TODO-BPJS.md) | **BPJS Integration Hub roadmap** — Fase BP0–BP8 (44 section, ~3.5-4.5 minggu). V-Claim 5 sub-menu + Aplicares 3 sub-menu + Audit. **BP0 ✅ + BP1 ✅ + BP2 ✅ Kepesertaan** (16 file `src/lib/bpjs/` + 4 file `src/components/bpjs/kepesertaan/`). Next: BP3 SEP. |
...
| `/ehis-bpjs`               | BPJS Integration Hub | 🚧 **BP0 ✅ + BP1 ✅ Beranda + BP2 ✅ Kepesertaan + BP3 ✅ SEP** — Lib: 16 file [src/lib/bpjs/](src/lib/bpjs/) · **BP2** ✅ 4 file [src/components/bpjs/kepesertaan/](src/components/bpjs/kepesertaan/) · **BP3 SEP** ✅ — 14 file [src/components/bpjs/sep/](src/components/bpjs/sep/): `SEPPage` **6-tab** scrollable (Cari·Update Tgl Pulang·Integrasi E-Klaim·Suplesi JR·SEP Internal·Fingerprint) + `useSearchParams ?update=` auto-switch · `CariSEPPanel`+`SEPDetailCard`+`HapusSEPModal` · `UpdateTglPulangForm`+`UpdateTglPulangList` · `SEPIntegrasiInaCBGCard` (iDRG/INA-CBG + deep-link eklaim) · `SuplesiJasaRaharjaForm`+`DataIndukKecelakaanForm` (amber/rose · `suplesiCek`+`dataIndukKecelakaan`) · `SEPInternalForm`+`SEPInternalList` (sky · inline hapus) · `FingerprintPanel`+`FingerprintListPanel` (indigo · compliance Permenkes 26/2021) · `sepShared` · TSC clean. **Next: BP4 Rujukan**. Roadmap [TODO-BPJS.md](TODO-BPJS.md) |
