...
`MappingHubPage.renderPane()` di-update untuk wire `kewenangan` & `layanan`. `SUBPAGE_REGISTRY` status: `kewenangan` & `layanan` → `"ready"` (sidebar pindah ke grup Aktif), `dependsOn` dihapus. Sub-page tersisa Coming Soon: `tarif`/`formularium`/`distribusi`/`rbac` (4 items).
