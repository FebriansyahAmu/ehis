...
| `/ehis-eklaim`             | E-Klaim          | 🚧 **EK0 Foundation 100% · EK1 Beranda 100% · EK2 Klaim Board 100% · EK3 Klaim Detail 29% (EK3.1 ✅ + EK3.2 ✅ Berkas)** (EK2: Hero teal · KPI Strip 4-card · FilterPanel 300px sticky · Quick Tabs · Sticky-header table 11 kolom · sort 3-state · Bulk bar sky slide-up · Kebab 9-aksi · Export CSV RFC 4180. **EK3.1: ClaimBannerHeader sticky 5-row + ClaimTabs 6-tab motion.layoutId + ClaimTimelineMini 5-stage animated pulse + Quick CTAs + 404 graceful + skeleton 500ms + route `[id]` server async params**. **EK3.2 Tab Berkas: 2-pane layout (5/12 list + 7/12 preview · stack di sm) · AutoPullBar 4 source (Resume Medis/Lab/Rad/Billing) dengan stagger pull + ExternalLink ke source modul · BerkasGroupList 4-group (Identitas/Klinis/Finansial/Khusus) progress per group + auto-collapse N.A + show optional toggle · BerkasRow status cycle clickable + Auto-pull/Upload/Replace/Preview/Catatan actions + notes textarea inline collapse · BerkasPreviewPane 3-mode (empty/no-file/file-viewer) + metadata strip 4-col + mock PDF/image preview + versions list + uploadedBy chip · ProgressBanner global emerald/amber · hidden file input upload stub dengan version append-only · 350ms latency simulasi**). 49 file total. **iDRG-first** (kode 7-digit · ICD-10-IM · KRIS · Pedoman iDRG 2025 + Perpres 59/2024). INA-CBG legacy adapter siap (parked). Next: EK3.3 Tab Koding (1.5 hari · ICD-10-IM + ICD-9-CM-IM picker). Roadmap [TODO-EKLAIM.md](TODO-EKLAIM.md) |
