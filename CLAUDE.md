...
`MappingHubPage.renderPane()` di-update untuk wire semua 7 sub-page. `SUBPAGE_REGISTRY` status: semua 7 sub-page → `"ready"`. `MappingHubSidebar` hide section "Segera Hadir" jika `soon.length === 0` (semua sudah aktif). ✅ TypeScript clean. Largest pane file: 275 lines (TarifPane), jauh di bawah 800 limit.
...
- [ ] **Import/export Excel** — Tarif Matrix paling butuh (admin billing biasa kerja via Excel). Kandidat: `xlsx` library + helper `tarifToWorkbook()` / `parseTarifWorkbook()`.
- [ ] **Audit trail** — siapa edit cell apa kapan. Kandidat shared `mapping/MappingAuditPane.tsx` consume audit log dari semua mapping operations.
