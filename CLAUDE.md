...
| [TODO-BPJS.md](TODO-BPJS.md) | **BPJS Integration Hub roadmap** — Fase BP0–BP8 (43 section, ~3.5-4.5 minggu). V-Claim 5 sub-menu + Aplicares 3 sub-menu + Audit. **BP0 Foundation 100%** ✅ (15 file `src/lib/bpjs/`). Next: BP1 Beranda BPJS. |
...
| `/ehis-bpjs`               | BPJS Integration Hub | 🚧 **BP0 ✅ Foundation + Spec Aligned (Trustmark BPJS)** — 16 file [src/lib/bpjs/](src/lib/bpjs/) compile clean. **BP0.1-0.2** 4 core + 11 domain types (SEPRecordExt restructure 1:1 spec: SEPKelasRawat enum literal · SEPPoli `{tujuan,eksekutif}` · SEPRujukan asalRujukan di dalam · SEPJaminan nested 3-level · 4 conditional rules validator). **BP0.3** 6 mock (12 peserta·20 SEP·15 rujukan·10RK+5SPRI·monitoring derived·aplicares dari RUANGAN_MOCK). **BP0.4** ~36 V-Claim method aligned [contracts/SEP-Contracts.md](contracts/SEP-Contracts.md): SEP CRUD + Suplesi Jasa Raharja + Data Induk Kecelakaan + Approval Penjamin (Pengajuan/Persetujuan) + Update Tgl Pulang + List + Integrasi Inacbgs + SEP Internal (GET+DELETE) + Finger Print (Get+List) + Random Question/Answer + Kepesertaan/Rujukan/Monitoring/RK/Aplicares. Removed sepInternalInsert/Update/sepJasaRaharja (tidak ada di spec). Schema separation: [bpjsShared.ts] domain types · [bpjsContracts.ts] wire payload/response 1:1 spec. **Audit-first enforced** via wrapWithAudit HOF + idempotencyKey per mutation. UI Phase BP1-BP8 belum dimulai. Roadmap [TODO-BPJS.md](TODO-BPJS.md) |
