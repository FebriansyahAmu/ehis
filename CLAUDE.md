...
| `/ehis-eklaim`             | E-Klaim          | 🚧 **EK0 ✅ · EK1 ✅ · EK2 ✅ · EK3 ✅ · EK4 ✅ · EK5 ✅ · EK6 ✅ · EK7 🚧 75% · EK8 ✅ 100%** — EK7 Reconciliation EK7.1+EK7.2+EK7.3 ✅ · **EK8 Dashboard** EK8.1–EK8.6 ✅: `DashboardPage` 2-panel (left nav 5 tab + right AnimatePresence) · KPI strip 4 cards · `ApprovalRatePanel` SVG line chart + rejected reasons · `AgingKlaimPanel` stacked bars + stuck claims table · `MarginAnalysisPanel` diverging bar chart · `CoderProductivityPanel` stacked daily bar + submit latency · **EK8.5** CSV export per 4 panel + `DashboardPrintTemplate` A4 · **EK8.6** `MarginComparatorPanel` iDRG vs INA-CBG SVG line + delta bars + MDC breakdown 10-row + per-penjamin filter + caveat banner + PDF watermark isolation · `dashboardShared.ts` pure builders · TSC clean. **iDRG-first**. Next: EK7.4 Reconciliation Report. Roadmap [TODO-EKLAIM.md](TODO-EKLAIM.md) |
