/**
 * Seed Katalog Keperawatan — data otoritatif awal untuk DB `master.sdki`.
 *
 * Dikonsumsi HANYA oleh prisma/scripts/seed-sdki.mts (bukan runtime aplikasi).
 * Aplikasi membaca dari DB via API (`@/lib/api/master/sdki`). File ini = "rumah"
 * data (dulu SDKI_MOCK di sdkiMock.ts), dipisah agar mock runtime bisa dihapus.
 *
 * - `id` = seedKey (sdki-dNNNN) → di-remap ke UUID saat seed.
 * - `kode` = kode resmi PPNI (D.NNNN) → di-seed APA ADANYA (bukan auto-gen).
 *   Counter `sdki_counter` di-set ke numerik tertinggi (148) → entri BARU via API
 *   mulai D.0149. Gap kode resmi (D.0002, dst.) sengaja tak diisi (impor penuh
 *   dataset PPNI = tahap terpisah, analog ICD bulk-import).
 *
 * Standar: PPNI SDKI (2017) · SLKI (2018) · SIKI (2018) edisi 1.
 * ~30 diagnosa representatif lintas 5 kategori (sampel; dataset penuh 149/119/240).
 */

import type { SdkiItem } from "./sdkiMock";

export const SDKI_SEED: SdkiItem[] = [
