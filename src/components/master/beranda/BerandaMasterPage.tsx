...
/**
 * Beranda Master — landing dashboard untuk modul EHIS Master.
 *
 * Sengaja TIDAK pakai `MasterPageLayout` (yang asumsi 2-panel list+detail).
 * Halaman ini = dashboard 12-kolom dengan:
 *   - Hero (eyebrow + title + description + timestamp pill)
 *   - KPIStrip (5 hero cards)
 *   - Grid 12: kiri (lg:col-span-8) = QuickNavGrid · kanan (lg:col-span-4) = Coverage + RecentEdits
 *
 * Loading state: skeleton selama `useSkeletonDelay(500)` → fade-in motion.
 */
...
import { getBerandaStats } from "./berandaShared";
import KPIStrip from "./KPIStrip";
import QuickNavGrid from "./QuickNavGrid";
import MappingCoveragePanel from "./MappingCoveragePanel";
    ...
    <div className="flex h-full flex-col gap-4 p-4 sm:p-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <div className="flex flex-col gap-3 lg:col-span-8">
          {Array.from({ length: 4 }).map((_, i) => <Bone key={i} className="h-32 rounded-xl" />)}
        </div>
...
export default function BerandaMasterPage() {
  const loaded = useSkeletonDelay(500);
  const stats = useMemo(() => getBerandaStats(), []);
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="flex h-6 w-6 items-center justify-center rounded-md bg-violet-50 ring-1 ring-violet-100">
                ...
                <p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-500 sm:text-[13px]">
                  Kelola semua data master rumah sakit dari satu tempat — sumber daya, katalog klinis,
                  reference standar, template, workflow, mapping antar entitas, dan konfigurasi.
                  Setiap perubahan tersinkron otomatis ke workflow klinis di seluruh modul EHIS.
                </p>
              </div>
