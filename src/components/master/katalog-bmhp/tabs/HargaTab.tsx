...
import { TrendingUp, TrendingDown, Wallet, BadgeCheck, type LucideIcon } from "lucide-react";
import { cn } from "@/lib/utils";
import {
  type BmhpRecord, type StatusBmhp,
  STATUS_BMHP_CFG,
} from "@/lib/master/bmhpMock";
import {
  Field, NumberInput, Select, SectionGroup,
} from "@/components/master/shared";
import { fmtIDR, calcMargin } from "../katalogBmhpShared";
    ...
    >
      ...
      >
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <Field label="HET (opsional)" hint="Batas eceran tertinggi">
            <NumberInput
              value={draft.het}
      ...
      <SectionGroup
        ...
        desc="Hanya barang status 'Aktif' yang muncul di pilihan order & billing."
      >
        <Field label="Status">
          <Select<StatusBmhp>
            value={draft.status ?? "Aktif"}
            onChange={(v) => v && onPatch({ status: v })}
            options={(Object.keys(STATUS_BMHP_CFG) as StatusBmhp[]).map((s) => ({
              value: s,
              label: STATUS_BMHP_CFG[s].label,
            }))}
            accent="teal"
            maxW="max-w-[240px]"
          />
