...
function DaftarKunjunganModal({
  ...
  const pjCfg = PENJAMIN_CFG[patient.penjamin.tipe];
  const isRujukan =
    caraMasuk === "Rujukan Puskesmas" || caraMasuk === "Rujukan RS";
        ...
        <div className="flex w-52 shrink-0 flex-col gap-2 border-r border-slate-100 bg-slate-50/80 p-3">
          ...
          <div className="mt-auto rounded-xl border border-amber-200 bg-amber-50 p-3">
            ...
            <p className="text-[10px] leading-relaxed text-amber-800">
              <span className="font-semibold">Lab & Radiologi</span> bukan
              kunjungan baru — dibuat sebagai order dari kunjungan aktif.
            </p>
          </div>
        </div>
          ...
          <div className="flex-1 space-y-5 overflow-y-auto p-5">
                ...
                {unit === "IGD" && (
                  <>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {["Jalan Kaki", "Ambulans", "Kendaraan Pribadi"].map(
                          (opt) => (
                            <button
                              key={opt}
            ...
            <div>
              ...
              >
                ...
                <span className="shrink-0 text-[9px] italic text-slate-400">
                  Dari rekam medis
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-4 md:col-span-3">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs">
              ...
              <div className="p-4 space-y-3">
                ...
                {/* CTA: Daftar Kunjungan Baru */}
                <button
                  onClick={() => setDaftarKunjungan(true)}
                  className="group flex w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-semibold text-white shadow-xs transition hover:bg-emerald-700 active:scale-[0.98]"
                >
                  <CalendarPlus
                    size={13}
