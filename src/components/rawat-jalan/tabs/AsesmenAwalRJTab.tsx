...
type SubTabId = "anamnesis" | "riwayat" | "alergi";
...
const SUB_TABS: SubTabDef[] = [
  { id: "anamnesis", label: "Anamnesis",    sublabel: "Keluhan & Keadaan Umum", icon: ClipboardList,  standard: "AP 1.1" },
  { id: "riwayat",   label: "Riwayat Medis", sublabel: "RPD · RPK · Obat",       icon: History,        standard: "AP 1.1" },
...
function SubNavItem({
  ...
  return (
    ...
    >
      ...
      {active && (
        <motion.div
          layoutId="rj-asesmen-active-bar"
      ...
      <div className="flex items-center justify-between mb-2">
        <div>
          <p className={cn("text-xs font-bold", allDone ? "text-emerald-800" : "text-sky-800")}>
            {allDone ? "Asesmen Awal Lengkap" : "Asesmen Awal Rawat Jalan"}
          </p>
          <p className={cn("text-[10px] mt-0.5", allDone ? "text-emerald-600" : "text-sky-600")}>
            {allDone
              ? "Semua komponen asesmen telah diisi — SNARS AP 1.1 terpenuhi"
              : `${doneCount} dari ${total} komponen selesai · SNARS AP 1.1`
            }
          </p>
        </div>
        <div className={cn(
...
export default function AsesmenAwalRJTab({ patient }: { patient: RJPatientDetail }) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
        ...
        <nav
          ...
          aria-label="Asesmen awal sub-tab"
        >
          {SUB_TABS.map(tab => (
            <SubNavItem
              key={tab.id}
