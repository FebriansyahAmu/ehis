...
// RJ TTV tab — wrapper yang mewiring TTVTab shared ke domain Observation (DB, unit-agnostic).
// Pasien DB (id = UUID) → fetch time-series saat mount + simpan via recordObservasi (mode
// multi-entry: shift dipilih perawat, waktu = now server). Pasien demo (mock) → teruskan tanpa
// onSave (perilaku in-memory shared). "Dicatat oleh" = user login (server otoritatif via actor;
// recordedBy hanya display). Sama backend dengan IGD/RI (gate clinical.rekammedis + careUnit).

import { useEffect, useState } from "react";
import { Loader2 } from "lucide-react";
import TTVTabShared, { type TTVSavePayload } from "@/components/shared/medical-records/TTVTab";
import type { RJPatientDetail, RITTVRecord, RIShift } from "@/lib/data";
...
// id kunjungan DB = UUID; id demo/mock ("rj-1") tak tersimpan ke DB.
...
export default function TTVTab({ patient }: { patient: RJPatientDetail }) {
  const { session } = useSession();
  const isPersisted = UUID_RE.test(patient.id);
  const [history, setHistory] = useState<RITTVRecord[] | null>(isPersisted ? null : []);
  const [loading, setLoading] = useState(isPersisted);

  // Muat time-series TTV dari DB (kunjungan nyata). Demo (mock) → mulai kosong.
  useEffect(() => {
  ...
  // Mode DB → simpan observasi (waktu = now server); kembalikan record terpersist ke shared
  // utk state lokal. Rawat Jalan: shift TIDAK dipakai (poliklinik) → tak dikirim.
  const handleSave = isPersisted
    ? async (p: TTVSavePayload): Promise<RITTVRecord> => {
        const dto = await recordObservasi(patient.id, {
          ...
          shift: undefined,   // RJ: shift irrelevan (poliklinik) — biar server tak paksa
          perawat: p.perawat, // fallback display; server otoritatif dari actor (user login)
          waktuObservasi: undefined, // tak dikirim → server = now (pencatatan live poli)
        });
        // Beri tahu komponen lain (header vitals bar) → update tanpa refresh.
