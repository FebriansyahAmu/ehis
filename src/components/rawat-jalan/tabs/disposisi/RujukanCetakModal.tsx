...
// Surat Rujukan Cetak Modal — overlay + toolbar (Cetak · tutup) + preview A4 interaktif.
// Pakai infra print global globals.css: `.print-area` (isolasi saat print) + `.no-print`.
// Pola identik SuratKontrolCetakModal. Data 1 surat rujukan (RujukanCetakData).

import { useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X, Printer } from "lucide-react";
import RujukanCetakTemplate, { type RujukanCetakData } from "./RujukanCetakTemplate";

interface Props {
  open: boolean;
  onClose: () => void;
  data: RujukanCetakData | null;
}

function ModalBody({ onClose, data }: { onClose: () => void; data: RujukanCetakData }) {
  useEffect(() => {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="surat-rujukan-print-title"
      >
        <header className="no-print flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 id="surat-rujukan-print-title" className="text-[13px] font-semibold text-slate-800">
                Cetak Surat Rujukan
              </h2>
              <p className="text-[10.5px] text-slate-500">
                Preview · A4 · {data.peserta.nama} · {data.peserta.noMr}
                <span className="ml-1 inline-flex items-center rounded bg-indigo-50 px-1 py-0.5 font-mono text-[9px] font-semibold text-indigo-700 ring-1 ring-indigo-200">
                  {data.noRujukan}
                </span>
              </p>
            </div>
        ...
        <div className="flex-1 overflow-auto bg-slate-200/60 px-3 py-5">
          <div className="print-area mx-auto w-[794px] max-w-full bg-white shadow-sm" data-paper="A4">
            <RujukanCetakTemplate data={data} />
          </div>
        </div>
...
export default function RujukanCetakModal({ open, onClose, data }: Props) {
  return (
    <AnimatePresence>{open && data && <ModalBody onClose={onClose} data={data} />}</AnimatePresence>
  );
}
