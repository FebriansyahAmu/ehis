...
/** InvoiceStateDTO (nyata) → InvoiceDetail (bentuk komponen desain). Read-only: tanpa diskon/void item. */
function toInvoiceDetail(s: InvoiceStateDTO): InvoiceDetail {
...
export default function KunjunganInvoiceDetail({ kunjunganId }: { kunjunganId: string }) {
  ...
  const detail = useMemo(() => (state ? toInvoiceDetail(state) : null), [state]);
