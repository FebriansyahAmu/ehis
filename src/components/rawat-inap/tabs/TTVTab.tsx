...
// RI TTV tab — wrapper yang mewiring TTVTab shared ke domain Observation (DB, unit-agnostic).
// Pasien DB (id = UUID) → fetch time-series saat mount + simpan via recordObservasi dalam
// mode RI multi-shift (shift dipilih perawat, waktu = now server). Pasien demo (mock) →
// teruskan data mock tanpa onSave (perilaku in-memory shared). Nama perawat = user login
// (server otoritatif via actor; recordedBy hanya display). Sama backend dengan IGD/RJ.

import { useEffect, useState } from "react";
import { Loader2 } from "lucide-react";
import TTVTabShared, { type TTVSavePayload } from "@/components/shared/medical-records/TTVTab";
import type { RawatInapPatientDetail, RITTVRecord, RIShift } from "@/lib/data";
...
// id kunjungan DB = UUID; id demo/mock ("ri-1") tak tersimpan ke DB.
...
export default function TTVTab({ patient }: { patient: RawatInapPatientDetail }) {
  ...
  // Mode DB → simpan observasi (RI multi-shift: shift dipilih, waktu = now server);
  // kembalikan record terpersist ke shared utk state lokal.
  const handleSave = isPersisted
    ? async (p: TTVSavePayload): Promise<RITTVRecord> => {
        const dto = await recordObservasi(patient.id, {
          ...
          shift: p.shift,     // RI: shift dipilih perawat
          perawat: p.perawat, // fallback display; server otoritatif dari actor (user login)
          waktuObservasi: undefined, // tak dikirim → server = now (pencatatan live bangsal)
        });
