...
// Modal pratinjau payload SPRI (JSON) — body wire V-Claim RencanaKontrol/InsertSPRI dari isian form
// penerbitan SPRI. Portal + Escape/backdrop tutup + tombol Salin. Pola identik RujukanPayloadModal.
// noKartu (penuh), kodeDokter (Mapping DPJP BPJS), & user di-resolve SERVER saat pengiriman
// (anti-spoof) → ini murni PRATINJAU dari isian form.
...
export default function SpriPayloadModal({
  payload,
  onClose,
}: {
  payload: unknown;
      ...
      >
        <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-[15px] font-bold text-slate-900">Payload SPRI</h2>
              <p className="text-[11px] text-slate-400">
                Pratinjau body <span className="font-mono">request</span> — V-Claim{" "}
                <span className="font-mono">RencanaKontrol/InsertSPRI</span>
              </p>
            </div>
          </div>
        ...
        <div className="border-t border-slate-100 bg-amber-50/60 px-5 py-2.5 text-[10px] leading-relaxed text-amber-700">
          Pratinjau isian form. <span className="font-mono">noKartu</span> ter-mask (server pakai nilai penuh dari penjamin);{" "}
          <span className="font-mono">kodeDokter</span> = kode DPJP BPJS dari Mapping Hub (kosong bila DPJP belum di-map);{" "}
          <span className="font-mono">user</span> = akun login. Nilai final tetap di-resolusi <span className="font-semibold">server</span> (anti-spoof).
        </div>
      </motion.div>
    </motion.div>,
    document.body,
