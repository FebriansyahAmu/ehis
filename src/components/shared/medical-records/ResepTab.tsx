...
import { Pill, CalendarCheck, BookOpen } from "lucide-react";
import type { ResepRIItem, MAREntry } from "@/lib/data";
import { cn } from "@/lib/utils";
import { type ResepPatient } from "@/components/shared/resep/resepShared";
import { getPIOLogsForRM } from "@/components/farmasi/pio/pioShared";

import ResepPane         from "./resep/ResepPane";
import MARPane           from "./resep/MARPane";
import PIOMintaInfoModal from "@/components/farmasi/pio/PIOMintaInfoModal";
import PIORiwayatPanel   from "@/components/farmasi/pio/PIORiwayatPanel";

// ── Types ─────────────────────────────────────────────────

type SubTab = "resep" | "mar" | "pio";
...
export default function ResepTab({ patient, showMAR = true }: Props) {
  ...
  const pioPending  = getPIOLogsForRM(patient.noRM).filter((l) => l.status === "Pending").length;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-xs">
        ...
        <button
          onClick={() => setShowPIOModal(true)}
          className="ml-auto flex items-center gap-1.5 rounded-lg border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-xs font-semibold text-sky-700 transition-colors hover:bg-sky-100"
        >
          <BookOpen size={11} />
          Minta Info Obat
        </button>
          ...
          {SUB_TABS.map(({ id, label, Icon }) => (
            <button
              ...
              className={cn(
                ...
                active === id ? "text-sky-700" : "text-slate-500 hover:text-slate-700",
              )}
            >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {active === "pio" && (
            <PIORiwayatPanel
              noRM={patient.noRM}
              onNewRequest={() => setShowPIOModal(true)}
            />
          )}
        </motion.div>
      </AnimatePresence>
        ...
        {showPIOModal && (
          <PIOMintaInfoModal
            patient={{
              ...
              name:        patient.name,
              dpjp:        patient.dpjp,
              perawatJaga: patient.perawatJaga,
            }}
            onClose={() => setShowPIOModal(false)}
          />
        )}
      </AnimatePresence>
    </div>
