...
// Surat Keterangan Sehat Cetak Modal — SHARED. Overlay + toolbar (picker surat bila >1 · Cetak ·
// tutup) + preview A4 interaktif. Pakai infra print global globals.css: `.print-area` (isolasi saat
// print) + `.no-print` (toolbar di-hide). Pola identik SuratSakitCetakModal.

import { useEffect, useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X, Printer, CheckCircle } from "lucide-react";
import { cn } from "@/lib/utils";
import SuratKeteranganSehatTemplate, { type SuratSehatCetakData } from "./SuratKeteranganSehatTemplate";

interface Props {
  open: boolean;
  onClose: () => void;
  /** 1 surat per keterangan sehat — urut terbaru dulu. */
  list: SuratSehatCetakData[];
}
...
function ModalBody({ onClose, list }: { onClose: () => void; list: SuratSehatCetakData[] }) {
  ...
  return (
    <>
      ...
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="surat-sehat-print-title"
      >
        {/* Toolbar */}
        <header className="no-print flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 bg-white px-4 py-2.5">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 id="surat-sehat-print-title" className="text-[13px] font-semibold text-slate-800">
                Cetak Surat Keterangan Sehat
              </h2>
              <p className="text-[10.5px] text-slate-500">
                Preview · A4 · {active.pasien.nama} · {active.pasien.noRM}
                <span className="ml-1 inline-flex items-center rounded bg-emerald-50 px-1 py-0.5 font-semibold text-emerald-700 ring-1 ring-emerald-200">
                  {active.surat.kesimpulan || "Sehat"}
                </span>
              </p>
            </div>
          </div>
        ...
        {list.length > 1 && (
          <div className="no-print flex flex-wrap items-center gap-1.5 border-b border-slate-200 bg-white px-4 py-2">
            ...
            {list.map((d, i) => (
              <button
                key={`${d.surat.nomor || d.surat.tglPeriksa}-${i}`}
                type="button"
                onClick={() => setIdx(i)}
                className={cn(
              ...
              >
                <CheckCircle size={11} className={i === idx ? "text-emerald-600" : "text-slate-400"} />
                <span className="font-mono font-semibold">{d.surat.nomor || fmtTglPendek(d.surat.tglPeriksa)}</span>
                <span className={i === idx ? "text-emerald-600/80" : "text-slate-400"}>
                  {fmtTglPendek(d.surat.tglPeriksa)} · {d.surat.kesimpulan || "Sehat"}
                </span>
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-auto bg-slate-200/60 px-3 py-5">
          <div className="print-area mx-auto w-[794px] max-w-full bg-white shadow-sm" data-paper="A4">
            <SuratKeteranganSehatTemplate data={active} />
          </div>
        </div>
...
export default function SuratSehatCetakModal({ open, onClose, list }: Props) {
  return (
    <AnimatePresence>
