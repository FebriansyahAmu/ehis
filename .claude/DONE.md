...
- Service [billingInvoiceService](../src/lib/services/billing/billingInvoiceService.ts): helper `itemNet`/`computeReduksi`/`applyItemAdjustments`; `buildState` overlay (Draft & Final, cocokkan sourceRef → set `diskonItem`/`voided`; `subtotal`=Σ net; `untariffedCount` net) · `getRingkas` byKategori net · `setItemAdjustment` (validasi baris ada + bertarif, blokir Final, upsert, audit `item.diskon`/`item.void`) · `removeItemAdjustment` (blokir Final, audit `item.unvoid`/`item.diskon.hapus` + `amount` = nilai yang dikembalikan). `listKunjunganBilling` kurangi Σ reduksi → board net.
...
- **Endpoint klinis BARU** `GET /kunjungan/:id/billing/ringkas` — [billingInvoiceService.getRingkas](../src/lib/services/billing/billingInvoiceService.ts) = slim `getInvoiceState` (invoiceId · status · penjaminTipe · subtotal · grandTotal · dibayar · sisa · untariffedCount) → DTO `BillingRingkasDTO` (schemas/billing/payment). Gate **`clinical.rekammedis:read`** (BUKAN `billing.invoice` yang hanya milik Kasir) → Dokter/Perawat (audience tab Pasien Pulang) boleh lihat sisa tagihan pasiennya; `scopeKunjungan` default ON (ABAC careUnit) karena resource `clinical.*`, action `read` lolos lock. Pola = klinisi baca lintas-domain via endpoint klinis (mis. `/lab/:id/hasil`). Client `getBillingRingkas`.
- **Hook bersama** [useBillingRingkas](../src/components/shared/medical-records/useBillingRingkas.ts) (UUID-guard demo vs DB · fetch + reaktif `useRecordVersion(kid,"order")` → order klinis baru/batal me-refetch sisa tanpa refresh · `billingKind()` derive no-invoice/lunas/outstanding). Menggantikan `useInvoiceDetail` (billingStore).
- **BillingMiniWidget** ([shared/medical-records](../src/components/shared/medical-records/BillingMiniWidget.tsx)) di-rewrite: prop `noRM`→**`kunjunganId`**, baca ringkas nyata, deep-link `/tagihan/kunjungan/[kid]` (bukan route mock `[id]`), + state loading. **BillingGateBanner** ([rawat-inap/pasienPulang](../src/components/rawat-inap/pasienPulang/BillingGateBanner.tsx)) idem: `kunjunganId`, 5 state (loading slate · no-invoice/demo slate · lunas emerald · outstanding **Umum** rose "Perlu Aksi" · **penjamin** sky "sisa ditanggung/klaim" + Cek E-Klaim). Call-site: [RIPatientHeader](../src/components/rawat-inap/RIPatientHeader.tsx) + [PasienPulangTab](../src/components/rawat-inap/tabs/PasienPulangTab.tsx) kirim `kunjunganId={patient.id}`.
- **Ingest client DICABUT** (dead write): `ingestLabOrder`/`ingestRadOrder`(×2)/`ingestFarmasiOrder` + import-nya dihapus dari [lab ValidasiPane](../src/components/lab/tabs/ValidasiPane.tsx) · [rad ValidasiPane](../src/components/rad/tabs/ValidasiPane.tsx) · [rad EkspertasiPane](../src/components/rad/tabs/EkspertasiPane.tsx) · [farmasi FarmasiOrderTabs](../src/components/farmasi/FarmasiOrderTabs.tsx). Charge = PROYEKSI order server-side → tulisan client ke mock store tak lagi dibaca di jalur RI.
- **TotalTagihanWidget re-source → selaras + Akomodasi ✅**: `getRingkas` diperluas `byKategori` (Σ item proyeksi per kategori = subtotal); DTO `BillingRingkasKategori`. [TotalTagihanWidget](../src/components/shared/medical-records/TotalTagihanWidget.tsx) (header IGD/RI/RJ) untuk pasien UUID **berhenti menjumlah 5 endpoint order klien** → 1 fetch `getBillingRingkas`, total = `subtotal`, popover per kategori proyeksi (**Akomodasi + Tindakan + Lab + Rad + Obat & BMHP + Jasa Dokter + Lain-lain**). Efek: **angka Total Tagihan RI kini termasuk akomodasi = identik dengan tagihan nyata** (`BillingMiniWidget` "Sisa" pakai grandTotal yang sama). Demo (non-UUID) tetap `ORDERS_MOCK` (Resep+BMHP→"Obat & BMHP"). Reaktivitas `recordBus "order"` dipertahankan.
- **Verifikasi:** `tsc` bersih (src) · `eslint` bersih (13 file) · pg smoke: `clinical.rekammedis:read` dimiliki **Admin/Dokter/Perawat** (gate tak mengunci audience) + data RI nyata (`RI/2026/00023` Completed inv `INV/2026/07/00001` dibayar 880.000; `RI/2026/00025` InService belum ada invoice).
- **Sisa (TECH_DEBT):** `billingStore`/`chargeIngest`/`sourceAdapter`/`invoiceMock` **masih** dipakai route mock lama `/tagihan/[id]` (InvoiceDetailPage) yang di-link eklaim (ClaimBannerHeader/berkasShared) + registration (BillingDetailModal) + Kasir (ChargeSummaryCard). Retire penuh = migrasi deep-link itu ke `/tagihan/kunjungan/[kid]` dulu, lalu hapus file mock.
