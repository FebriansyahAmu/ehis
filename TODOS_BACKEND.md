...
### B1.1 Sumber Daya
- [✅] **`/api/v1/master/pegawai`** — Pegawai (HR/SDM) **layered penuh (2026-06-04)**: schema `master.Pegawai` (+ JadwalShift/Absensi/PegawaiKontakDarurat, eks-`sdm`, Jabatan/Golongan dibuang) · Zod [pegawai.ts](src/lib/schemas/pegawai.ts) · DAL [pegawaiDal.ts](src/lib/dal/pegawaiDal.ts) (cursor list+trigram, version-guard, soft-delete) · Service [pegawaiService.ts](src/lib/services/pegawaiService.ts) (PII NIK enc/hash, dedup NIK+NIP, clock-injected DTO mask) · Route [route.ts](src/app/api/v1/master/pegawai/route.ts)+[\[id\]](src/app/api/v1/master/pegawai/[id]/route.ts) (GET list/detail · POST · PATCH · DELETE soft) · **Unit test ✅** [pegawaiService.test.ts](src/lib/services/pegawaiService.test.ts) (18 test: dedup NIK/NIP · version guard · DTO mask · guard tanggal · profesi→isDokter). **Contract align + wiring FE ✅ (2026-06-04):** enum `StatusPegawai` → `ASN/Outsourcing/Honorer/Magang/Mitra` (migrasi data-preserving remap PNS|PPPK→ASN, Kontrak→Outsourcing) + kolom baru `agama` & `profesi` (sumber kebenaran Dokter/Perawat/…; `isDokter` diturunkan dari profesi∈{Dokter,Dokter Gigi,Dokter Spesialis} ∨ practitionerId). API client [pegawai.ts](src/lib/api/pegawai.ts) (form→CreateInput mapper) · wizard **Tambah Pengguna Step 1 WIRED** ke `POST /api/v1/master/pegawai` (error CONFLICT/VALIDATION tampil di banner). **Sisa:** NIP auto-generate non-ASN (butuh counter/sequence) · scopeBy(actor) saat auth siap · cek akun-yatim saat delete (use-case auth) · integration test DAL (Testcontainers).

> **Pengguna wizard Step 2 (Akun) & Step 3 (Peran) masih MOCK** — butuh modul auth provisioning (hash argon2id · userDal/userService transaksi create user+roles+unitscope · Zod · route `POST /auth/users` + assign roles · seed Role). Schema `auth.*` sudah ada; implementasi belum. Lihat [Phase B0/B-Auth](#).
