...
- **Tabel** `master.TarifTindakan` (FK `tindakan`) · `master.TarifLabTest` (FK `labTest`) · `master.TarifRadCatalog` (FK `radCatalog`) — semuanya uuid v7, timestamptz, `@@unique(<itemId>, penjaminKode, jenisRuangan)` → **upsert by triple**; FK Restrict; `harga` Int rupiah. Migrasi `20260612060000_init_master_tarif_tindakan` · `20260617140000_init_master_tarif_lab_test` · `20260617170000_init_master_tarif_rad_catalog`.
