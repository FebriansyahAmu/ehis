...
- **Batas (TECH_DEBT)**: **settings saja** — `billingProjectionService` **belum** membaca `TarifKamar`/`TarifAdministrasi` (akomodasi tetap `AKOMODASI_RATE` hardcode; administrasi belum jadi charge). Wiring proyeksi = fase berikutnya (tutup gap #5 & #1 nyata).
