...
## 5. Tarif Matrix — Tindakan/Lab × Penjamin × Jenis Ruangan ✅

Matriks 3D di-flatten jadi baris edge `(item, penjaminKode, jenisRuangan) → harga`. Baris = **federasi Katalog Tindakan + Katalog Laboratorium** (grup "Tindakan Laboratorium"), persis Layanan Unit. Tiap domain = **tabel berdiri-sendiri** (bukan polymorphic), selaras `LayananUnit` ⇄ `LayananUnitLab`.

- **Tabel** `master.TarifTindakan` (FK `tindakan`) · `master.TarifLabTest` (FK `labTest`) — keduanya uuid v7, timestamptz, `@@unique(<itemId>, penjaminKode, jenisRuangan)` → **upsert by triple**; FK Restrict; `harga` Int rupiah. Migrasi `20260612060000_init_master_tarif_tindakan` · `20260617140000_init_master_tarif_lab_test`.
- **Endpoint** (gate **`master.tarif`**): `GET /master/tarif-tindakan` · `GET /master/tarif-lab-test` (filter+cursor) · `POST` (upsert by triple, action `update`) · `DELETE /:id` (clear → "belum diisi"). Endpoint persist dipilih per **`kind` baris** (tindakan vs lab).
- **Lapisan** schemas/DAL (`upsert`/keyset list)/Service (`list` actor-less · `upsert` guard item · `remove`) + client `lib/api/master/tarifTindakan.ts` · `tarifLabTest.ts` (`listAllTarif*`/`upsert*`/`delete*`).
- **FE** [tarif/](../src/components/master/mapping/tarif/) — **baris federasi** Tindakan (per kategori) + Lab (grup Lab) via `rowsFromTindakan`/`rowsFromLab` (reuse `layananShared`); `TarifMap` di-key by **rowId** (tindakanId/labTestId), edge 2 endpoint dinormalisasi `TarifEdgeLike` (SSR-hybrid). **kolom = tier "Jenis Ruangan"** DI-DERIVE dari master Ruangan tree (`tiersFromTree`, key `locationType[:kelas]`) — BUKAN Location fisik (tarif per kelas, bukan per bed). Penjamin = sheet selector (daftar **khusus Tarif** `TARIF_PENJAMIN`, BUKAN `PENJAMIN_MOCK` global): **Umum** aktif (= Tarif PERDA, berlaku semua jaminan) · **BPJS** tab nonaktif/disabled (KRIS ditunda) · jaminan lain dihapus. `penjaminKode` tetap string stabil. Sel = harga, edit inline → upsert/delete optimistik + reconcile (dispatch per kind). **KRIS-aware**: `validTiersForPenjamin` collapse tier inap berkelas → 1 tier `RAWAT_INAP:KRIS` untuk BPJS; Umum/Asuransi tetap VIP/Kelas (Perpres 59/2024). **Flat-rate per baris**: tombol `=` per baris (popover input 1 harga) → samakan ke SEMUA tier sheet aktif (item ber-harga seragam lintas ruangan, mis. pasang infus/EKG/tes lab) — batched upsert.
- **Keputusan model**: derive tier dari tree · KRIS+klasik per penjamin · penjamin = kode string (master Penjamin backend menyusul Tier 3 — `penjaminKode` belum FK) · **Tarif Lab = tabel paralel `TarifLabTest`** (2026-06-17, bukan polymorphic — selaras LayananUnitLab; tes lab dibilling per tier seperti tindakan, umumnya flat-rate).
- **Sisa**: master **Penjamin** masih mock (kode stabil dipakai) · bulk-adjust = loop upsert (belum endpoint bulk) · Rad belum di-federasi (menu Order tersendiri) · federasi billable Tindakan+Lab+Rad → chargemaster (TODO-CHARGEMASTER CM2/CM5).
