...
| **Radiologi** ✅ (2026-06-23) | `receive` · `saveHasil` (penulis ekspertise; **finalize = ekspertise + validasi + rilis sekaligus → Selesai**) · `saveAkuisisi` (radiografer pelaksana) — semua enforce `assertActorAssignedToRad` | [radAssignment.ts](../src/lib/services/rad/radAssignment.ts) di `radOrderService.receive` + `radResultService.saveHasil` + `radAkuisisiService.saveAkuisisi`. **Ekspertise & Validasi disatukan**: validator = radiolog penulis (penanda tangan TTE); `validate` lama disisakan sbg legacy. Nama radiolog = free-text desain pane; **radiografer akuisisi = multi-pick dari roster** (anti-spoof); aktor tetap wajib ter-assign |
