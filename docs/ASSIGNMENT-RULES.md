...
| **Radiologi** ✅ (2026-06-23) | `receive` · `saveHasil` (penulis ekspertise) · `validate` (aktor validator) · `saveAkuisisi` (radiografer pelaksana) — semua enforce `assertActorAssignedToRad` | [radAssignment.ts](../src/lib/services/rad/radAssignment.ts) di `radOrderService.receive` + `radResultService.saveHasil/validate` + `radAkuisisiService.saveAkuisisi`. Nama radiolog/validator = free-text desain pane; **radiografer akuisisi = multi-pick dari roster** (nama diturunkan dari roster, anti-spoof); aktor tetap wajib ter-assign |
