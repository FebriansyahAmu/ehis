...
| 9   | **Keperawatan** (RM)     | NursingCare       | ✅  | 🟢  | 🟢     | Fase A+B ✅ (`medicalrecord.AsuhanKeperawatan` CRUD + verify co-sign + evaluasi shift JSONB; gate **`clinical.keperawatan`** Perawat penulis utama; wired IGD). Template dari **DB `master.sdki`** via `sdki-template` (gate klinis). tanggal=DateTimePicker · perawat=sesi login. SDKI/SLKI/SIKI PPNI |
...
**Model `medicalrecord.AsuhanKeperawatan`** (keyed `kunjunganId`, shared IGD/RI/RJ): **CRUD** — 1 baris = 1 diagnosa keperawatan (proses asuhan). Edit, **verify** (co-sign supervisor/DPJP), **evaluasi shift** (timeline JSONB, append via PATCH), hapus = soft-delete (entered-in-error). Blok pengkajian (dataMayor/dataMinor) + intervensi SIKI + evaluasi = JSONB; `kriteriaHasil` (SLKI) = text[]. `kodeSdki` = soft-ref `master.sdki.kode` (snapshot saat pilih template). `tanggalInput` = timestamptz (DateTimePicker); `perawat` = denormalisasi sesi login. Mirror pola TindakanMedis. PPNI SDKI 2017 / SLKI 2018 / SIKI 2018.
...
- ⚠️ **Sisa (follow-up):** wiring RI/RJ (RI punya copy `AsuhanForm`/`AsuhanCard` sendiri di `rawat-inap/tabs/keperawatan/` — belum dimigrasi) · `SDKI_CATALOG` mock (15 entri) tetap sbg fallback offline · evaluasi sub-form perawat masih input bebas (bisa di-default sesi) · import penuh dataset PPNI.
