...
| 15  | **Resep Pasien** (Lyn)   | Order (REUSE)     | ✅  | ⬜  | ⬜     | MedicationRequest. **FE decision-support ✅ (2026-06-18, shared IGD+RI+RJ):** dropdown global `Select` (Depo + Signa/Waktu/Rute) · **Dosis sekali minum** (`ResepRIItem.dosisSekali?`) · **peringatan alergi obat dari anamnesis** (`getAlergiObat` baca `ALLERGY_MOCK` kat. Obat + teks bebas `riwayatAlergi`; `matchAlergiObat` sadar-golongan mis. Penisilin→Amoxicillin) → banner + warning saat pilih/tambah + badge per-baris · **3 dropdown kondisi klinis** (Fungsi Ginjal selalu; Kehamilan & Menyusui khusus ♀) · **no. kontak DPJP** di bawah nama penulis (default "-"). Konstanta/helper + `KondisiKlinisPanel`/`AlergiObatBanner` di shared `resep/`; IGD pakai [ResepPasienTab](src/components/igd/tabs/ResepPasienTab.tsx) sendiri, RI/RJ pakai [ResepPane](src/components/shared/medical-records/resep/ResepPane.tsx)+[ResepItemRow](src/components/shared/resep/ResepItemRow.tsx). BE/Wiring tetap ⬜ (domain Order belum dibangun) |
