...
- [ ] **No. SKDP = No. Referensi SPRI — pemilih SPRI** *(FE ✅ sebagian)* — [SpriPickerModal](src/components/registration/patient/modals/daftar-kunjungan/sep/SpriPickerModal.tsx): tombol "Pilih SPRI" → modal tabel, **auto-search by No. Kartu BPJS** + filter tanggal, pilih baris → isi `skdpNoSurat = spri.noReferensi`. **Gap:** (a) search masih **CLIENT-side** (`listSpri({})` default = MenungguRef+Terbit, filter `noKartu` di FE) → butuh endpoint server **`GET /spri?noKartu=`** + sertakan semua status (kontrol pasca-ranap bisa `Dikonsumsi`); (b) SPRI tanpa `noReferensi` (MenungguRef) masih bisa dipilih (ref kosong) → guard saat terbit.
...
- [ ] **🔴 DPJP otomatis ke payload — RANTAI PUTUS (2 titik)** — payload punya **2** kode DPJP beda sumber: **`skdp.kodeDPJP`** = DPJP pada Surat Kontrol → dari **SPRI terpilih**; **`dpjpLayan`** = DPJP yang melayani → **kosong saat RANAP** (`jnsPelayanan="1"`), hanya RJ. Field **"Kode DPJP (SKDP)" DIHAPUS** dari form (per user) → harus diisi saat build. **Rantai ideal:** `SPRI.dpjpPegawaiId → master.Dokter → kode DPJP BPJS → skdp.kodeDPJP`. **Blocker (putus di 2 titik):** (1) [SpriDTO](src/lib/schemas/disposisi/disposisi.ts#L86) **tak ekspos `dpjpPegawaiId`** (kolom ada di [encounter.prisma:264](prisma/schema/encounter.prisma#L264) + tipe DAL, tapi DTO cuma `dpjpNama` string → tak joinable); (2) **`master.Dokter` tak punya kolom kode DPJP BPJS** — yang ada `ihsPractitionerId` (**SatuSehat**, bukan BPJS), STR/SIP, spesialisKode. Kode DPJP V-Claim = dari **"referensi dokter DPJP" BPJS** (per faskes/poli), belum disimpan. **Fix (pola [[project_wilayah_strategy]] — sync referensi, jangan lookup live):** (A) sync ref dokter DPJP BPJS + mapping `Dokter↔kodeDpjpBpjs` (atau kolom baru di master.Dokter, set per poli); (B) ekspos `dpjpPegawaiId` di SpriDTO mapper; (C) build: `pickedSpri.dpjpPegawaiId→Dokter→kodeDpjpBpjs→skdpKodeDPJP`, guard `dpjpLayan` kosong utk RANAP. Catatan: `pickedSpri` kini hanya state lokal SepFormBody (tak persist ke draft) → simpan referensi SPRI terpilih juga. Lihat gap "DPJP≠SPRI by nama".
- [ ] **No. Telepon prefill dari pasien** — field **tetap ditampilkan** (editable, hint "dapat diubah"), tapi `sepDraft.noTelp` **belum di-prefill** dari `Pasien.noHp` saat masuk Step SEP. Wire prefill dari data pasien (just-in-case ada perubahan nomor).
...
- [ ] **`dpjpLayan` harus kosong saat RANAP** — payload: "tidak diisi jika jnsPelayanan=1". Form sudah menyembunyikan field DPJP Pelayanan untuk RANAP (RJ-only), tapi [buildRegisterInput](src/components/registration/patient/modals/daftar-kunjungan/daftarKunjunganApi.ts) mengirim `dpjpLayan: orUndef(sepDraft.dpjpLayan)` **tanpa guard unit** (nilai bisa terbawa bila RJ→RI). Tambah guard `jnsPelayanan==="2"`.
- [ ] **Adapter t_sep bersarang (V-Claim `insertSEP`)** — `SepInput` saat ini **flat**; payload resmi bersarang (`t_sep.klsRawat / rujukan / poli / cob / katarak / jaminan.penjamin.suplesi.lokasiLaka / skdp`). Saat wire V-Claim nyata (ganti mock SEP), buat adapter flat→nested + map nilai (kelas, lakaLantas, dst). Kini SEP = mock (lihat [docs/MOCK-JKN-KEPESERTAAN.md] & item MOCK SEP).
