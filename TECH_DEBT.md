...
- [ ] **DPJP otomatis dari SPRI ke payload** — field **"Kode DPJP (SKDP)" DIHAPUS** dari form (per user). Saat build payload, `skdpKodeDPJP`/`dpjpLayan` harus ditarik dari SPRI terpilih. **Blocker:** [SpriDTO](src/lib/schemas/disposisi/disposisi.ts#L86) hanya ekspos `dpjpNama` (tanpa kode DPJP **BPJS**); butuh map DPJP→kode BPJS + simpan referensi SPRI terpilih (saat ini `pickedSpri` hanya state lokal SepFormBody, tak persist ke draft). Lihat juga gap "DPJP≠SPRI by nama".
