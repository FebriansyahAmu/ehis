...
- [ ] **🔴 `kodeDokter` + `poliKontrol` BPJS belum ada** — payload InsertSPRI butuh **kode dokter DPJP BPJS** + **kode poli BPJS**; kini dikirim **kosong** (`""`). Akar sama dgn gap `skdp.kodeDPJP` (lihat §Step SEP) — butuh **mapping internal→BPJS** (Dokter↔kodeDpjpBpjs, poli↔kodeBpjs) via referensi sync (BWS3 [TODO-BPJS-WS.md](TODO-BPJS-WS.md)). Mock tak peduli (selalu sukses); real WAJIB diisi.
