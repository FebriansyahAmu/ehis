...
- [ ] **Unit Kerja multi-pilih disimpan koma-join (interim)** — field Unit Kerja di [PenggunaAddWizard](src/components/master/pengguna/PenggunaAddWizard.tsx) Step 1 + [PegawaiEditModal](src/components/master/pengguna/PegawaiEditModal.tsx) kini `MultiSelect` (1 pegawai bisa lintas IGD+RI+ICU), tapi backend `master.Pegawai.unitKerja` tetap **satu String** → disimpan gabungan dipisah koma via `splitUnitKerja`/`joinUnitKerja` ([penggunaShared.ts](src/components/master/pengguna/penggunaShared.ts)). **Konflik halus** dgn keputusan arsitektur 2026-05-22 (penugasan operasional N:N = sumber kebenaran di **Mapping Hub**, bukan field HR). **Saat backend Mapping Hub dibangun:** promosikan multi-unit ke relasi/array yang benar (`UserUnitScope`/assignment table), `unitKerja` kembali jadi unit "rumah" tunggal saja. Risiko interim: tak joinable utk query "siapa di unit X" (string LIKE), dan koma di nama unit akan rusak (saat ini tak ada). Lihat juga [UserUnitScope belum diisi](#) di atas + Mapping Hub bidirectional sync.
