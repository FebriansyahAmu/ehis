...
- [ ] **Workflow status order DB belum persist** — `medicalrecord.ResepOrder.status`/`prioritas` ditulis saat create (`Menunggu`) tapi transisi Farmasi (Telaah→Dispensasi→Serah) BELUM ada endpoint PATCH. [FarmasiBoard](src/components/farmasi/FarmasiBoard.tsx) menampilkan order DB **read-only** (action card menggerakkan `workflowStore` in-memory yang hanya berlaku untuk order MOCK, bukan order DB). Fix closed-loop: endpoint `PATCH /farmasi/resep/:id/status` (gate `ancillary.farmasi.telaah`/`ancillary.farmasi.serah`) + Telaah/SerahTerima persist + invalidasi board. Sampai itu, status order DB tetap di "Menunggu".
